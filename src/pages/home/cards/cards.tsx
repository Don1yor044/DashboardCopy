...
export const Cards = observer(() => {
  ...
  const setTotalPrice = searchStore.setTotalPrice;
  const setTotalPaymentFee = searchStore.setTotalPaymentFee;
  ...
  useEffect(() => {
    setTotalPrice(
      totalPaidByCard + totalPaidByCash + totalPaidByPayme + totalPaidByDiscount
    );
  }, [
    totalPaidByCard,
    totalPaidByCash,
  ...
  const fetchSearchResults = async (token: string) => {
    ...
    const response = await baseURL.get(`/api/client/dashboard/all/${search}`, {
